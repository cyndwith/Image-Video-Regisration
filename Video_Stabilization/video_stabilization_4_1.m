...
%function[]=video_stabilization_4_1(filename)
    ...
    for i=2:noFrames
        ...
        Trans=estimateGeometricTransform(Apoints,Bpoints,'affine');
        temp=Trans.T;
        ...
        newX(i) = newT(3,1);
        newY(i) = newT(3,2);
        ATrans= imwarp(A,affine2d(newT),'OutputView',outputView);
        ATrans = imresize(ATrans,[video.Height,video.Width]);
        u(:,:,:,i)= ATrans;%autoCrop(BTrans);%.*uint8(maskIm));
    end
    figure;
